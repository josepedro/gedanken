...
Nr = 6;                    % Number of lines   (cells in the y direction)
Mc = 6;                    % Number of columns (cells in the x direction)
...
rho_l = 0.0001;   % initial disturbance
f(Nr/2, Mc/2,:) = rho_l;
f(:,:,1);
...
for ta = 1 : 1%150*sqrt(3)
    ...
    feq(:,:,1)= rt1 .*(1 +f1*ux +f2.*uxsq -f3*usq); % vert 3
    feq(:,:,2)= rt1 .*(1 +f1*uy +f2*uysq -f3*usq); % vert 5
    feq(:,:,3)= rt1 .*(1 -f1*ux +f2*uxsq -f3*usq); % vert 7
    feq(:,:,4)= rt1 .*(1 -f1*uy +f2*uysq -f3*usq); % vert 1
    feq(:,:,5)= rt2 .*(1 +f1*(+ux+uy) +f2*(+ux+uy).^2 -f3.*usq); % diag 2
    feq(:,:,6)= rt2 .*(1 +f1*(-ux+uy) +f2*(-ux+uy).^2 -f3.*usq); % diag 4
    feq(:,:,7)= rt2 .*(1 +f1*(-ux-uy) +f2*(-ux-uy).^2 -f3.*usq); % diag 6
    feq(:,:,8)= rt2 .*(1 +f1*(+ux-uy) +f2*(+ux-uy).^2 -f3.*usq); % diag 8
    feq(:,:,9)= rt0 .*(1 - f3*usq); % center
